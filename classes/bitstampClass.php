...
class Bitstamp
{
        ...
        public $eur_usd;   // Current eur/usd
        /**
        ...
        public function bitstamp_query_public($url)
        {
                // our curl handle (initialize if required)
                static $ch = null;
                if (is_null($ch))
         ...
         */
        function ticker() {
                $ticker = $this->bitstamp_query_public('https://www.bitstamp.net/api/ticker/');
                $this->ticker = $ticker; // Another variable to contain it.
                return $ticker;

        }
        function eur_usd() {
                $eur_usd = $this->bitstamp_query_public('https://www.bitstamp.net/api/eur_usd/');
                $this->eur_usd = $eur_usd; // Another variable to contain it.
                return $eur_usd;
        }

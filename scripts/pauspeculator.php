...
//---------------Empezamos-------------------------------
$metodo='macd';
$slowPeriod=26;		 // Standard 26
$fastPeriod=12;		// Standar 12
$signalPeriod=9;	// Standard 9
$margenMacd=0.98;
$vela=60;
$salida="";
...
$salida.=$fechaHora.' ['.$mailUser.'] ['.$exchange.'] ['.$ecoin.'] :'; //string de salida del script
...
if ($metodo == "media") {
	...
	$valores = array_reverse($oTrade->getArrayPrecios($fechaHora,$vela,$slowPeriod+$signalPeriod-1));
...
switch ($metodo) {
	case "media":
		$decisionCompra = ($precioMediaSmall>=$precioMediaLarge);
		break;
	case "macd":
		if ($ecoinModo=='venta'):
			$decisionCompra = ($histograma >= -($margenMacd));
		elseif ($ecoinModo=='compra'):
			$decisionCompra = ($histograma >= $margenMacd);
		endif;
		break;
	default:
